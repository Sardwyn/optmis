...
export function renderScrapedProducts(products) {
    ...
    const rows = products.map((p, i) => {
      ...
      const toggleBtn = hasVariants
        ? `<button class="variant-toggle" data-target="variant-${i}" style="margin-right:5px;">+</button>`
        : '';
  
      const variantsHtml = hasVariants
        ? `<ul id="variant-${i}" class="variant-list" style="display:none; margin: 5px 0 0 0; padding-left: 16px;">` +
          p.variants.map(v =>
      ...
      return `
        <tr>
          ...
          <td>
            ${toggleBtn}
            <a href="${p.url}" target="_blank">${truncatedTitle}</a>
            ${variantsHtml}
          </td>
          <td>${p.price || '–'}</td>
          <td>${p.sku || ''}</td>
          <td>${p.description?.substring(0, 80) || ''}</td>
          <td>${hasVariants ? `${p.variants.length} variants` : ''}</td>
        </tr>`;
    }).join('');
