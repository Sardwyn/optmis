...
function pss_maybe_rewrite_description($description, $product_data, $openai_api_key) {
    if (empty($openai_api_key)) {
        error_log('❌ Missing OpenAI API key. Cannot evaluate or rewrite description.');
        return $description;
    }

    // Case 1: Empty description → generate from scratch
    if (empty($description)) {
        error_log('💡 Description is empty. Generating new one via AI.');
        $generated = pss_generate_ai_description($product_data, $openai_api_key);
        return $generated ?: 'Full product description coming soon. Contact us for more details if you need more information on this product.';
    }

    // Case 2: Evaluate existing quality and rewrite if weak
    ...
    $rating = 10; // Default to "pass" if AI fails
    ...
    // Rewrite if rating is 6 or lower
    if ($rating <= 6) {
        error_log("📝 Description rated {$rating}/10 — rewriting via AI.");
        $rewritten = pss_generate_ai_description($product_data, $openai_api_key);
...
function pss_generate_ai_description($p, $api) {
    ...
    // ✅ Check for quota exceeded AFTER parsing
    if (isset($body['error']['code']) && $body['error']['code'] === 'insufficient_quota') {
        error_log('❌ OpenAI quota exceeded. Using fallback message.');
        return 'Full product description coming soon. Contact us for more details if you need more information on this product.';
    }

    if (empty($body['choices'][0]['message']['content'])) {
        error_log('⚠️ Unexpected AI response structure. Raw: ' . print_r($body, true));
        return '';
    }
