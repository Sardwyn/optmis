jQuery(document).ready(function ($) {
  ...
  const val = stored[this.value] ?? 0;
  $('#pss-price-modifier').val(val);
});
    ...
    fetch(pssScraperData.ajaxUrl, {
      ...
      .then(result => {
        ...
        categories.forEach(cat => {
          ...
          $row.append($select, $saveBtn, $scrapeBtn);
...
$('#pss-import-selected').on('click', async function () {
  ...
  const supplierSlug = $('#pss-supplier-select').val();
const modifier = parseFloat($('#pss-price-modifier').val()) || 0;
...
selectedProducts.forEach(p => {
  ...
  if (p.price) {
    ...
    if (!isNaN(numeric)) {
      const adjusted = numeric * (1 + modifier / 100);
...
}); // <-- this closing brace & paren are critical
