...
export async function parse({ url, category }, fetchPage) {
  ...
  for (const el of items) {
    try {
      ...
      const sku = rawSku.replace(/\*$/, ''); // remove trailing * if present
      ...
      // Always visit PDP to check for variants
      const pdp = await fetchPage(href);
      console.log('🧾 PDP snippet:', pdp.body?.innerHTML?.slice(0, 1000));
      console.log('🧬 Checking for variant selectors on PDP');
      console.log('Swatches found:', pdp.querySelectorAll('.swatch-option').length);
      console.log('Selects found:', pdp.querySelectorAll('select.super-attribute-select').length);
      ...
      // Dropdowns
      pdp.querySelectorAll('select.super-attribute-select').forEach(select => {
        const options = [...select.querySelectorAll('option')];
        options.forEach(opt => {
          const val = opt.value.trim();
          const text = opt.textContent.trim();
